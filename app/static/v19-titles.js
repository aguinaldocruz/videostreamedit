...
renderMovies = function () {
  ...
  $('#movie-list').innerHTML = files.map(file => `<tr><td><strong class="movie-title title-with-alternatives" title="${attr(alternativeTitleHint(file))}">${esc(movieTitle(file))}${file.note ? ` <span class="note-tag" title="${attr(file.note)}">i</span>` : ''}${file.reviewed ? ` <span class="reviewed-tag" title="Reviewed">✓</span>` : ''}${file.plex_sync_change ? ` <span class="plex-sync-tag" title="Plex Sync Change">P</span>` : ''}${typeof reportConfidenceDot === 'function' ? reportConfidenceDot(file.portuguese_detection_confidence,file.portuguese_detection_metadata,file.portuguese_detection_language) : ''}</strong></td><td>${esc(file.root_name)}</td><td>${bytes(file.size)}</td><td><button class="edit-file" data-path="${attr(file.path)}" data-label="${attr(movieTitle(file))}">Stream properties</button></td></tr>`).join('');
  wireEditors();
};
...
renderShows = function () {
  ...
  $('#show-list').innerHTML = shows.map(show => `<button class="show-card ${state.currentShow?.id === show.id ? 'active' : ''}" data-id="${attr(show.id)}"><strong class="title-with-alternatives" title="${attr(alternativeTitleHint(show))}">${esc(clean(show.name))}${show.note ? ` <span class="note-tag" title="${attr(show.note)}">i</span>` : ''}${show.reviewed ? ` <span class="reviewed-tag" title="Reviewed">✓</span>` : ''}${show.plex_sync_change ? ` <span class="plex-sync-tag" title="Plex Sync Change">P</span>` : ''}${typeof reportConfidenceDot === 'function' ? reportConfidenceDot(show.portuguese_detection_confidence,show.portuguese_detection_metadata,show.portuguese_detection_language) : ''}</strong><small>${show.episode_count} episodes · ${esc(show.root_name)}</small></button>`).join('');
  document.querySelectorAll('.show-card').forEach(button => button.onclick = () => {state.currentShow = state.shows.find(show => show.id === button.dataset.id);state.currentSeason = '*';$('#episode-search').value = '';window._preserveShowSelection = true;renderShows();renderEpisodes()});
};
