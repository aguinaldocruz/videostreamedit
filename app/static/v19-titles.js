...
renderMovies = function () {
  ...
  $('#movie-list').innerHTML = files.map(file => `<tr><td><strong class="movie-title title-with-alternatives" title="${attr(alternativeTitleHint(file))}">${esc(movieTitle(file))}${file.note ? ` <span class="note-tag" title="${attr(file.note)}">i</span>` : ''}${file.reviewed ? ` <span class="reviewed-tag" title="Reviewed">✓</span>` : ''}${file.plex_sync_change ? ` <span class="plex-sync-tag" title="Plex Sync Change">P</span>` : ''}${typeof reportConfidenceDot === 'function' ? reportConfidenceDot(file.portuguese_detection_confidence,file.portuguese_detection_metadata,file.portuguese_detection_language,'subtitle',file.portuguese_detection_no_confidence) : ''}</strong></td><td>${esc(file.root_name)}</td><td>${bytes(file.size)}</td><td><button class="edit-file" data-path="${attr(file.path)}" data-label="${attr(movieTitle(file))}">Stream properties</button></td></tr>`).join('');
  wireEditors();
};
