...
function ensureMovieImportUi() {
  ...
  $('#import-output-filename').oninput = updateImportSelection;
...
function selectImportMovie(button) {document.querySelectorAll('[data-import-file]').forEach(item=>item.classList.toggle('active',item===button));movieImportMode={source:button.dataset.importFile,sourceName:button.dataset.name,editing:false};$('#import-output-filename').value=button.dataset.name;updateImportSelection()}

function updateImportSelection() {const destination=document.querySelector('[name=import-destination]:checked')?.value,filename=$('#import-output-filename')?.value.trim();if(movieImportMode){movieImportMode.destination=destination;movieImportMode.filename=filename}const valid=Boolean(movieImportMode?.source&&destination&&filename);$('#import-edit-copy').disabled=!valid;$('#import-selection-summary').textContent=valid?`${movieImportMode.sourceName} → ${destination}/${filename}`:'Choose a source movie and destination.'}
