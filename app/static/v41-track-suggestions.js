(function () {
  ...
  function applySuggestions(suggestions, panel) {
    const active = document.activeElement;
    document.querySelectorAll('#stream-content .stream-row').forEach(row => {
      const input = row.querySelector('[name=title]');
      const suggestion = suggestions.find(item => item.old_value === input.value.trim());
      if (!suggestion || row.dataset.codecType !== suggestions[0].stream_type || row.querySelector('[name=remove]')?.checked) return;
      input.value = suggestion.new_value;
      input.dataset.dirty = 'true';
      input.classList.add('learned-track-name-suggestion');
      input.title = `Suggested from ${suggestion.use_count} previous ${suggestion.stream_type} corrections`;
    });
    closeSavedValueMenu();
    ...
    toast(`${suggestions[0].stream_type === 'audio' ? 'Audio' : 'Subtitle'} track-name suggestions queued for review`);
  }

  function renderSuggestionGroup(streamType, suggestions) {
    if (!suggestions.length) return;
    const label = streamType === 'audio' ? 'Audio' : 'Subtitle';
    const descriptions = suggestions.map(item => {
      const from = item.old_value || '(empty)';
      return `<li>“${esc(from)}” → “${esc(item.new_value)}” <small>${item.use_count} previous ${esc(streamType)} changes</small></li>`;
    }).join('');
    const panel = document.createElement('div');
    panel.className = 'track-name-suggestion';
    panel.dataset.suggestionType = streamType;
    panel.innerHTML = `<div><strong>${label} track-name correction available</strong><p>${label} and subtitle correction memories are kept separately. This only fills matching ${streamType} fields for review.</p><ul>${descriptions}</ul></div><div><button type="button" data-dismiss-suggestion>Not now</button><button type="button" class="primary" data-use-suggestion>Use ${label.toLowerCase()} suggestions</button></div>`;
    ...
    panel.querySelector('[data-use-suggestion]').onclick = () => applySuggestions(suggestions, panel);
  }
  ...
  function renderTrackNameSuggestions(items) {
    ...
    for (const streamType of ['subtitle', 'audio']) {
      renderSuggestionGroup(streamType, suggestions.filter(item => item.stream_type === streamType));
    }
  }
