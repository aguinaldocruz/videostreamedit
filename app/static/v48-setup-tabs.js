(function () {
  ...
  panel('language-detection').innerHTML = `<article class="language-detection-settings"><h3>Common languages</h3><p>Only subtitles whose metadata matches one of these common language bases (or is empty/undetermined) are inspected. The local detector uses built-in vocabulary profiles for Portuguese and English; other configured languages are retained for filtering but are not deeply analyzed.</p><label>Common languages<input id="language-detection-common" type="text" placeholder="pt, pt-BR, en" autocomplete="off"></label><button type="button" class="primary" id="language-detection-save">Save common languages</button><small id="language-detection-status"></small><hr><h3>Voice language detection</h3><p>Optional audio-language checks using the separate SpeechBrain Docker service.</p><label class="voice-setting-toggle"><input id="voice-detection-enabled" type="checkbox"> Enable voice detection interface</label><div id="voice-detection-config"><label>Service URL<input id="voice-detection-url" type="text" placeholder="http://language-id:9000"></label><label>Sample seconds<input id="voice-detection-seconds" type="number" min="10" max="120" step="1"></label><label>Sample positions<input id="voice-detection-positions" type="text" placeholder="0.1, 0.5, 0.9"></label></div><button type="button" class="primary" id="voice-detection-save">Save voice settings</button><button type="button" id="voice-detection-help">Docker instructions</button><small id="voice-detection-status"></small><hr><h3>Detection queue</h3><p>Run both subtitle and voice-language detection through the existing queues. A full run clears only previous detection results, then queues every movie and episode. Changed-media mode keeps detection enabled for future stream or metadata changes without scanning the current library.</p><div class="language-detection-queue-actions"><button type="button" class="primary" id="language-detection-queue-full">Queue full detection</button><button type="button" id="language-detection-queue-incremental">Enable changed-media detection</button></div><label class="voice-setting-toggle"><input id="language-detection-incremental" type="checkbox"> Automatically queue detection after media changes</label><small id="language-detection-queue-status"></small><hr><h3>Forced streams report exclusions</h3><p>Manage track names to omit from the Forced streams report. Matching is case-insensitive.</p><div class="forced-exclusion-add"><input id="forced-report-exclusion-new" type="text" placeholder="Track name to exclude" autocomplete="off"><button type="button" id="forced-report-exclusion-add" class="primary">Add track name</button></div><div id="forced-report-exclusion-list"></div><small id="forced-report-exclusions-status"></small><hr><h3>OCR conversion staging</h3><p>Converted media is placed at its final media path for review. The original remains staged until approval; if you reject the conversion, rollback restores the staged original. Approval only removes rollback and temporary data.</p><small id="ocr-stage-root" class="muted"></small><div id="ocr-staged-list"><p class="muted">Loading staged conversions…</p></div><hr><h3>Voice metadata report</h3><p>Audio streams whose sampled spoken language disagrees with the stored stream language.</p><button type="button" class="primary" id="audio-language-report-open">Refresh voice report</button><small id="audio-language-report-status"></small><div id="audio-language-report-list"><p class="muted">No report loaded.</p></div></article>`;
  ...
  function activateSetupTab(name) {
    ...
    if (name === 'language-detection') loadOcrStaged();
  }
  ...
  function renderOcrStaged(items) {
    const root=$('#ocr-staged-list');
    if (!items.length) {root.innerHTML='<p class="muted">No OCR staging backups awaiting review.</p>';return;}
    root.innerHTML=items.map(item=>`<div class="ocr-staged-row" data-id="${item.id}"><div><strong>${esc(item.title||item.original_path)}</strong><small>${esc(item.media_kind||'media')} · ${esc(item.original_path)} · ${esc(item.status)} · original backup ${Math.max(0,Math.round((Number(item.size_bytes)||0)/1048576))} MB</small></div><div class="ocr-staged-actions">${item.converted_path?`<a class="button" href="/api/v68/ocr/staged/${item.id}/converted" target="_blank" rel="noopener">Review converted</a>`:''}${item.converted_path&&item.converted_path!==item.original_path?'<button type="button" data-ocr-finalize>Move converted to media path</button>':''}<button type="button" data-ocr-rollback>Rollback to original</button><button type="button" class="danger" data-ocr-approve>Approve &amp; remove rollback data</button></div></div>`).join('');
    root.querySelectorAll('.ocr-staged-row').forEach(row=>{
      const id=row.dataset.id;
      row.querySelector('[data-ocr-finalize]')?.addEventListener('click',async()=>{if(!confirm('Move the converted candidate to the media’s original path? The original backup will remain available for rollback until approved.'))return;try{await api(`/api/v68/ocr/staged/${id}/finalize`,{method:'POST'});toast('Converted media is now at the original media path');loadOcrStaged()}catch(error){toast(error.message,true)}});
      row.querySelector('[data-ocr-rollback]').onclick=async()=>{if(!confirm('Restore the original media from staging? The staged backup will be kept until approval.'))return;try{await api(`/api/v68/ocr/staged/${id}/rollback`,{method:'POST'});toast('Original media restored');loadOcrStaged()}catch(error){toast(error.message,true)}};
      row.querySelector('[data-ocr-approve]').onclick=async()=>{if(!confirm('Approve this conversion and delete the staged original/rollback data? The media at its original path will be kept.'))return;try{await api(`/api/v68/ocr/staged/${id}/approve`,{method:'POST'});toast('Rollback data removed; final media retained');loadOcrStaged()}catch(error){toast(error.message,true)}};
    });
  }
  async function loadOcrStaged(){try{const result=await api('/api/v68/ocr/staged');$('#ocr-stage-root').textContent=`Staging area: ${result.stage_root}`;renderOcrStaged(result.items||[])}catch(error){$('#ocr-staged-list').innerHTML=`<p class="error">${esc(error.message)}</p>`}}
