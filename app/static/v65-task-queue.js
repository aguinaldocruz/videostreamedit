(function () {
  ...
  function render(data){
    const counts=data.counts||{};const statusMarkup=`<div class="queue-status-summary"><button type="button" class="queue-status-filter${activeStatus==='running'?' active':''}" data-queue-status="running" aria-pressed="${activeStatus==='running'}">${counts.running||0} running</button><button type="button" class="queue-status-filter${activeStatus==='pending'&&!activeType?' active':''}" data-queue-status="pending" aria-pressed="${activeStatus==='pending'&&!activeType}">${counts.pending||0} pending</button><button type="button" class="queue-status-filter${activeStatus==='failed'?' active':''}" data-queue-status="failed" aria-pressed="${activeStatus==='failed'}">${counts.failed||0} failed</button><button type="button" class="queue-status-filter${activeStatus==='succeeded'?' active':''}" data-queue-status="succeeded" aria-pressed="${activeStatus==='succeeded'}">${counts.succeeded||0} completed</button></div>`;const typeMarkup=`<div class="queue-type-summary"><span class="queue-summary-label">Pending by type</span>${(data.pending_types||[]).map(item=>`<button type="button" class="queue-type-filter${activeType===item.task_type?' active':''}" data-queue-type="${esc(item.task_type)}" aria-pressed="${activeType===item.task_type}">${esc(item.task_type.replace(/_/g,' '))} (${item.count})</button>`).join('')}</div>`;summary.innerHTML=statusMarkup+typeMarkup;summary.querySelectorAll('[data-queue-status]').forEach(button=>button.onclick=()=>{const status=button.dataset.queueStatus;activeStatus=activeStatus===status?null:status;activeType=null;loadQueue()});summary.querySelectorAll('[data-queue-type]').forEach(button=>button.onclick=()=>{const type=button.dataset.queueType;activeType=activeType===type?null:type;activeStatus=activeType?'pending':null;loadQueue()});control.textContent=data.paused?'Resume queue':'Pause queue';control.classList.toggle('primary',data.paused);
