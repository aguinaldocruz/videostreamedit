(function () {
  ...
  maintenance.querySelectorAll('[data-index-job]').forEach(card=>{
    ...
    card.insertAdjacentHTML('beforeend',`<div class="index-schedule"><label>Scheduled incremental check<select data-index-frequency><option value="disabled">Disabled</option><option value="daily">Daily</option><option value="every_other_day">Every other day</option><option value="weekly">Weekly</option></select></label><label>Time<input type="time" data-index-time value="03:00"></label><button type="button" data-index-schedule-save>Save schedule</button><small data-index-schedule-status>Loading schedule…</small></div>`);
