(function(){
  ...
  async function expand(){
    ...
    try{const result=await api('/api/v79/tv/season-stream-values',{method:'POST',body:JSON.stringify({paths:episodes.map(item=>item.path)})});window.currentTvStreamRecords=records=result.values;scope=currentScope();renderFilters();if(result.errors.length)toast(`${result.errors.length} episodes could not be indexed`,true)}catch(error){filterContent.innerHTML=`<div class="season-filter-loading error">${esc(error.message)}</div>`}finally{loading=false;toggle.disabled=false}
  }
