function openReportMediaEditor(path,label,navigationItems,reportDialog,onReturn){
  ...
  const items=(navigationItems||[]).filter(item=>item?.path);
...
function reportConfidenceDot(value,metadata,detected,kind='subtitle',noConfidence=false){if(kind==='audio'&&window.voiceDetectionEnabled===false)return '';const uncertain=kind!=='audio'&&noConfidence?'<span class="language-detection-no-confidence" title="Subtitle language could not be determined confidently; text may be empty, damaged, advertising, or unsupported">×</span>':'';if(value==null||Number(value)<.6)return uncertain;const pct=(Number(value)*100).toFixed(1);const color=typeof streamDetectionColor==='function'?streamDetectionColor(detected,metadata,'',value):reportConfidenceColor(value);const matches=typeof streamDetectionMatches==='function'&&streamDetectionMatches(detected,metadata,'');const label=matches?'Detected language matches stream metadata':(kind==='audio'?'Voice language mismatch detected':'Subtitle language metadata mismatch detected');return uncertain+'<span class="language-detection-dot" style="--detection-color:'+color+'" title="'+label+' with '+pct+'% confidence"></span>'}
...
(function(){
  ...
  async function load(){
    try{
      const result=await api('/api/v79/language-detection/health');
      const stream=result.stream_status||{},queue=result.queue||{};
    ...
    }catch(error){panel.innerHTML=`<strong>Subtitle inspection health</strong><span class="error">${esc(error.message)}</span>`}
  }
  load();
...
(function(){
  ...
  async function queueAllHtml(button){button.disabled=true;button.textContent='Queueing…';if(typeof beginGlobalBusy==='function')beginGlobalBusy('Queueing HTML cleanup');if(typeof setGlobalBusyProgress==='function')setGlobalBusyProgress(0,3,'Queueing HTML cleanup','Preparing all eligible report entries');const streams=currentItems.flatMap(item=>item.streams||[]);if(!streams.length){button.textContent='No eligible streams';if(typeof endGlobalBusy==='function')endGlobalBusy();return}if(!confirm(`Queue HTML tag removal for ${streams.length} subtitle stream${streams.length===1?'':'s'}?`)){button.disabled=false;button.textContent=`Queue HTML cleanup (${streams.length})`;if(typeof endGlobalBusy==='function')endGlobalBusy();return}try{if(typeof setGlobalBusyProgress==='function')setGlobalBusyProgress(1,3,'Queueing HTML cleanup','Submitting subtitle cleanup requests');const result=await api('/api/v19/reports/subtitle-action',{method:'POST',body:JSON.stringify({action:'html_cleanup',streams})});if(typeof setGlobalBusyProgress==='function')setGlobalBusyProgress(3,3,'HTML cleanup queued',`${result.queued||0} operation(s) queued`);button.textContent=`Queued ${result.queued||0}`;toast(`${result.queued||0} HTML subtitle cleanup operation${result.queued===1?'':'s'} queued`)}catch(error){button.disabled=false;button.textContent=`Queue HTML cleanup (${streams.length})`;toast(error.message,true)}finally{if(typeof endGlobalBusy==='function')endGlobalBusy()}}
  async function openSubtitleReport(button,type,kind){languageFix.hidden=true;if(htmlFix){htmlFix.hidden=true;htmlFix.disabled=false}currentKind=kind;currentType=type;const isHtml=type==='html',label=isHtml?'HTML':'image',action=isHtml?'html_cleanup':'image_convert';title.textContent=isHtml?(kind==='tv'?'TV Shows with HTML subtitles':'Movies with HTML subtitles'):(kind==='tv'?'TV Shows with Image subtitles':'Movies with Image subtitles');summary.textContent='Loading indexed report…';items.innerHTML='<p class="reports-loading vse-status" data-status="pending">Loading…</p>';dialog.showModal();button.disabled=true;try{const result=await api(`/api/v19/reports/${isHtml?'html-subtitles':'image-subtitles'}?kind=${encodeURIComponent(kind)}`);currentItems=result.items||[];summary.textContent=`${result.title_count} titles · ${result.media_count} media with ${label} subtitles`;if(isHtml&&htmlFix){const count=(result.items||[]).reduce((total,item)=>total+(item.streams||[]).length,0);htmlFix.hidden=count===0;htmlFix.textContent=`Queue HTML cleanup (${count})`;htmlFix.onclick=()=>queueAllHtml(htmlFix)}renderSubtitleItems(label,action)}catch(error){summary.textContent='';items.innerHTML=`<p class="reports-error vse-status" data-status="failed">${esc(error.message)}</p>`}finally{button.disabled=false}}
  ...
  function openForcedEditor(path,label,navigationItems){openReportMediaEditor(path,label,navigationItems,dialog)}
  function openForcedReport(button,kind){languageFix.hidden=true;if(htmlFix)htmlFix.hidden=true;currentKind=kind;currentType='forced';title.textContent=kind==='tv'?'TV Shows with Forced streams':'Movies with Forced streams';summary.textContent='Loading indexed report…';items.innerHTML='<p class="reports-loading vse-status" data-status="pending">Loading…</p>';dialog.showModal();button.disabled=true;api(`/api/v19/reports/forced-streams?kind=${encodeURIComponent(kind)}`).then(result=>{currentItems=result.items||[];const forcedNavigation=forcedReportNavigationItems(currentItems,kind);summary.textContent=`${result.title_count} titles · ${result.media_count} media with Forced streams`;items.innerHTML=currentItems.length?currentItems.map((item,index)=>{const details=item.episodes?item.episodes.map((ep,ei)=>`<div class="report-subitem"><button type="button" class="report-episode-link vse-link" data-forced-episode="${index}:${ei}">${esc(ep.episode)}</button><span>${ep.forced_count} forced stream${ep.forced_count===1?'':'s'}</span></div>`).join(''):`<span>${item.forced_count} forced stream${item.forced_count===1?'':'s'}</span>`;return `<div class="report-item vse-panel"><button type="button" class="report-title-link vse-link" data-forced-title="${index}">${esc(item.title)}</button><div class="report-details">${details}${item.root_name?`<small>${esc(item.root_name)}</small>`:''}</div>${!item.episodes&&item.path?`<button type="button" class="report-item-action vse-btn" data-forced-edit="${index}">Stream properties</button>`:''}</div>`}).join(''):'<p class="reports-empty vse-status" data-status="current">No forced streams found.</p>';items.querySelectorAll('[data-forced-title]').forEach(b=>b.onclick=()=>openListedMedia(currentItems[Number(b.dataset.forcedTitle)]));items.querySelectorAll('[data-forced-edit]').forEach(b=>b.onclick=()=>{const item=currentItems[Number(b.dataset.forcedEdit)];openForcedEditor(item.path,item.title,forcedNavigation)});items.querySelectorAll('[data-forced-episode]').forEach(b=>b.onclick=()=>{const [i,e]=b.dataset.forcedEpisode.split(':').map(Number),ep=currentItems[i]?.episodes?.[e];if(ep){openForcedEditor(ep.path,`${currentItems[i].title} · ${ep.episode}`,forcedNavigation)}})}).catch(error=>{items.innerHTML=`<p class="reports-error vse-status" data-status="failed">${esc(error.message)}</p>`}).finally(()=>button.disabled=false)}
...
(function(){
  ...
  api('/api/v19/settings/duplicate-languages').then(result=>{audio.value=(result.audio||[]).join(', ');subtitle.value=(result.subtitle||[]).join(', ')}).catch(error=>{status.textContent=error.message});
  save.onclick=async()=>{save.disabled=true;try{const result=await api('/api/v19/settings/duplicate-languages',{method:'PUT',body:JSON.stringify({audio:audio.value.split(',').map(x=>x.trim()).filter(Boolean),subtitle:subtitle.value.split(',').map(x=>x.trim()).filter(Boolean)})});audio.value=(result.audio||[]).join(', ');subtitle.value=(result.subtitle||[]).join(', ');status.textContent='Saved.';toast('Duplicate-language report settings saved')}catch(error){status.textContent=error.message;toast(error.message,true)}finally{save.disabled=false}};
})();
...
(function(){
  ...
  save?.addEventListener('click',async()=>{save.disabled=true;try{const result=await api('/api/v79/audio-language-detection/settings',{method:'PUT',body:JSON.stringify({enabled:enabled.checked,service_url:url.value,sample_seconds:Number(seconds.value||30),sample_count:Number(count?.value||3),sample_positions:positions.value.split(',').map(Number).filter(Number.isFinite)})});window.voiceDetectionEnabled=result.enabled;status.textContent='Voice detection settings saved.';apply();if(window.loadMovies)loadMovies();if(window.loadTv)loadTv();toast('Voice detection settings saved')}catch(error){status.textContent=error.message;toast(error.message,true)}finally{save.disabled=false}});
...
(function(){
  ...
  function render(result,kind){
    ...
    (result.items||[]).forEach((item, index)=>{
      if(kind==='tv' && Array.isArray(item.episodes)){
        item.episodes.forEach(ep=>rows.push(`<div class="report-item vse-panel"><span class="report-title-link vse-link">${esc(item.title||'')} · ${esc(ep.episode||'')}</span><span>${(ep.streams||[]).length} damaged subtitle stream${(ep.streams||[]).length===1?'':'s'}</span><button type="button" class="report-item-action vse-btn" data-damage-edit="${rows.length}">Stream properties</button></div>`));
      } else {
        rows.push(`<div class="report-item vse-panel"><span class="report-title-link vse-link">${esc(item.title||'')}</span><span>${Number(item.damaged_subtitle_count||0)} damaged subtitle stream${Number(item.damaged_subtitle_count||0)===1?'':'s'}</span><button type="button" class="report-item-action vse-btn" data-damage-edit="${rows.length}">Stream properties</button></div>`);
      }
    });
    items.innerHTML=rows.join('')||'<p class="reports-empty vse-status" data-status="current">No damaged SRT subtitles found.</p>';
    let n=0;
    (result.items||[]).forEach(item=>{
      if(kind==='tv' && Array.isArray(item.episodes)) item.episodes.forEach(ep=>{const b=items.querySelectorAll('[data-damage-edit]')[n++];if(b)b.onclick=()=>edit(ep.path,`${item.title||''} · ${ep.episode||''}`,uniqueReportNavigation((result.items||[]).flatMap(group=>group.episodes||[])))});
      else {const b=items.querySelectorAll('[data-damage-edit]')[n++];if(b)b.onclick=()=>edit((item.paths||[])[0],item.title,uniqueReportNavigation((result.items||[]).flatMap(group=>(group.paths||[]).map(path=>({path,label:group.title})))))}
    });
  }
  document.querySelectorAll('[data-damaged-subtitle-report]').forEach(button=>button.onclick=async()=>{
    ...
    try{const result=await api('/api/v19/reports/damaged-subtitles?kind='+encodeURIComponent(kind));summary.textContent=`${result.title_count||0} titles · ${result.media_count||0} media`;render(result,kind)}
    catch(error){summary.textContent='';items.innerHTML=`<p class="reports-errors vse-status" data-status="failed">${esc(error.message||'Unable to load report')}</p>`}
...
(function(){
  ...
  document.querySelectorAll('[data-duplicate-language-report]').forEach(button=>button.onclick=async()=>{
    ...
    try{
      const result=await api(`/api/v19/reports/duplicate-languages?kind=${encodeURIComponent(kind)}&stream_type=${encodeURIComponent(type)}`);
      ...
      const entries=[];
      (result.items||[]).forEach(item=>{const media=item.episodes||[{path:item.path,label:item.title}];media.forEach(ep=>(ep.duplicates||[]).forEach(duplicate=>entries.push({path:ep.path,label:ep.episode?`${item.title} · ${ep.episode}`:item.title,duplicate})))});
      items.innerHTML=entries.map((entry,index)=>`<div class="report-item vse-panel" data-dup-row="${index}"><span class="report-title-link vse-link">${esc(entry.label)}</span><span>${esc(entry.duplicate.language)} ×${entry.duplicate.count}</span><button type="button" class="report-item-action vse-btn" data-dup-edit="${index}">Stream properties</button><button type="button" data-dup-ignore="${index}" title="Remove from report until the relevant streams change">Dismiss</button></div>`).join('')||'<p class="reports-empty vse-status" data-status="current">No duplicate-language media found.</p>';
...
(function(){
  ...
  document.querySelectorAll('[data-no-confidence-report]').forEach(button=>button.onclick=async()=>{
    ...
    try{
      const result=await api(`/api/v19/reports/subtitle-no-confidence?kind=${encodeURIComponent(kind)}`);
...
(function(){
  ...
  function render(){
    items.innerHTML=rows.length?rows.map((item,index)=>`<div class="report-item vse-panel"><div><strong>${esc(item.title)}</strong>${item.show_title?`<small>${esc(item.show_title)}</small>`:''}<span>${esc(item.source)} subtitle ${Number(item.type_index)>=0?Number(item.type_index)+1:''} · ${esc(item.status)} · ${esc(item.reason||'No reason recorded')}</span><details><summary>Evidence</summary><small>${item.cue_count||0} cues · ${item.text_chars||0} text chars · ${Number(item.text_coverage||0).toFixed(1)}% coverage · ${item.markup_count||0} markup tags${item.damage?` · ${esc(item.damage)}`:''}<br>Metadata: ${esc(item.metadata_language||'empty')}${item.metadata_region?` / ${esc(item.metadata_region)}`:''}${item.external_path?`<br>External: ${esc(item.external_path)}`:''}</small></details></div><div class="report-item-actions"><button type="button" class="report-item-action vse-btn" data-no-confidence-edit="${index}">Stream properties</button><button type="button" class="report-item-action vse-btn" data-no-confidence-revalidate="${index}">Revalidate</button></div></div>`).join(''):'<p class="reports-empty vse-status" data-status="current">No subtitle streams match the selected review filters.</p>';
    items.querySelectorAll('[data-no-confidence-edit]').forEach(node=>node.onclick=()=>{const item=rows[Number(node.dataset.noConfidenceEdit)];openReportMediaEditor(item.path,item.show_title?`${item.show_title} · ${item.title}`:item.title,rows.map(row=>({path:row.path,label:row.show_title?`${row.show_title} · ${row.title}`:row.title})),dialog)});
    items.querySelectorAll('[data-no-confidence-revalidate]').forEach(node=>node.onclick=async()=>{const item=rows[Number(node.dataset.noConfidenceRevalidate)];node.disabled=true;node.textContent='Queued…';try{const result=await api('/api/v19/reports/subtitle-revalidate',{method:'POST',body:JSON.stringify({paths:[item.path],subtitle_indices:item.type_index>=0?[item.type_index]:null})});node.textContent=result.queued?'Queued':'Already queued';toast(result.queued?'Subtitle revalidation queued':'Subtitle inspection already queued')}catch(error){node.disabled=false;node.textContent='Revalidate';toast(error.message,true)}});
  }
  async function load(){
    ...
    try{const query=new URLSearchParams({kind:currentKind});if(status)query.set('status',status);if(reason)query.set('reason',reason);const result=await api('/api/v19/reports/subtitle-no-confidence?'+query.toString());rows=result.items||[];summary.textContent=`${result.media_count||0} media · ${result.stream_count||0} subtitle streams`;render()}catch(error){summary.textContent='';items.innerHTML=`<p class="reports-error vse-status" data-status="failed">${esc(error.message)}</p>`}
  }
