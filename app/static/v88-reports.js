...
window.showReportPreflight=async function(dialog,ids){
  ...
  for(let attempt=0;attempt<8;attempt++){try{const statuses=await Promise.all(values.map(id=>api('/api/v89/preflight/'+encodeURIComponent(id))));render(statuses);if(statuses.some(item=>item.status==='pending'||item.status==='running'))await new Promise(resolve=>setTimeout(resolve,750));else return}catch(error){content.textContent=error.message;return}}
};
...
(function(){
  ...
  document.querySelectorAll('[data-duplicate-language-report]').forEach(button=>button.onclick=async()=>{
    ...
    try{
      ...
      const entries=[];
      (result.items||[]).forEach(item=>{const media=item.episodes||[{path:item.path,label:item.title}];media.forEach(ep=>(ep.duplicates||[]).forEach(duplicate=>entries.push({path:ep.path,label:ep.episode?`${item.title} · ${ep.episode}`:item.title,show:item.title,episode:ep.episode,duplicate}))) });
      if(kind==='tv'){
...
(function(){
  ...
  function escText(value){return typeof esc==='function'?esc(value):String(value??'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]))}
  ...
  document.querySelectorAll('[data-uncommon-language-report]').forEach(button=>button.onclick=async()=>{const kind=button.dataset.uncommonLanguageReport;title.textContent=kind==='tv'?'TV Shows with uncommon languages':'Movies with uncommon languages';summary.textContent='Loading indexed report…';items.innerHTML='<p class="reports-loading vse-status" data-status="pending">Loading…</p>';reportDialog.showModal();button.disabled=true;try{const result=await api(`/api/v19/reports/uncommon-languages?kind=${encodeURIComponent(kind)}`);renderReport(result,kind)}catch(error){items.innerHTML=`<p class="reports-error vse-status" data-status="failed">${escText(error.message)}</p>`}finally{button.disabled=false}});
})();
