...
$('#stream-form').onsubmit=async function(e){
  ...
  try{
    setApplyProgress(2,4,'Updating media container',queuedChangeSummary());
    const result=await api('/api/v7/media/edit',{method:'POST',body:JSON.stringify({path,tracks,external_subtitles:external,order,default_audio:defaults.audio,forced_audio:forced.audio,default_subtitle:defaults.subtitle,forced_subtitle:forced.subtitle,remove})});
    setApplyProgress(3,4,'Saving reusable values','Checking repeated metadata values');await offerSavedValues(usedValues);setApplyProgress(4,4,'Refreshing properties','Reading updated streams from the media file');toast(result.warnings.length?result.warnings.join(' '):queued+' change'+(queued===1?'':'s')+' applied',result.warnings.length>0);await openEditor(path,label);document.dispatchEvent(new CustomEvent('media-properties-applied',{detail:{path}}));applySucceeded=true;
